...
@section('content')
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('relatorios.mensal') }}" class="row g-2 align-items-end">
                ...
                <div class="col-md-3">
                    ...
                    <a href="{{ route('relatorios.pdf', ['ano' => $ano]) }}" class="btn btn-outline-danger" target="_blank"><i class="fa-solid fa-file-pdf me-1"></i>Exportar PDF</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-header bg-gradient-primary text-white">
            <h5 class="mb-0"><i class="fa-solid fa-scale-balanced me-2"></i>Balanço Financeiro {{ $ano }}</h5>
        </div>
            ...
            <table class="table table-sm table-bordered align-middle mb-0">
                ...
                <tbody>
                    <tr>
                        <th>Receitas</th>
                        @foreach (range(1, 12) as $m)
                            <td class="text-end text-success">{{ number_format($totaisMesReceitas[$m], 2, ',', '.') }}</td>
                        @endforeach
                    ...
                    <tr>
                        <th>Despesas</th>
                        @foreach (range(1, 12) as $m)
                            <td class="text-end text-danger">{{ number_format($totaisMesDespesas[$m], 2, ',', '.') }}</td>
                        @endforeach
                    ...
                    <tr class="table-secondary">
                        <th>Saldo</th>
                        @foreach (range(1, 12) as $m)
                            <td class="text-end fw-bold {{ $saldoMes[$m] >= 0 ? 'text-success' : 'text-danger' }}">
                                {{ number_format($saldoMes[$m], 2, ',', '.') }}
                            </td>
                        @endforeach
                ...
                <tbody>
                    @foreach (['receitas' => 'Receitas', 'despesas' => 'Despesas'] as $tipo => $titulo)
                        <tr class="{{ $tipo === 'receitas' ? 'table-success' : 'table-danger' }}">
                            <td colspan="14"><strong>{{ $titulo }}</strong></td>
                        </tr>
                        @forelse ($agrupadas[$tipo] ?? [] as $categoriaNome => $itens)
                            ...
                            @endforeach
                        @empty
                            <tr><td colspan="14" class="text-center text-muted">Nenhum lançamento de {{ $titulo }} em {{ $ano }}.</td></tr>
                        @endforelse
                    @endforeach
                </tbody>
            </table>
